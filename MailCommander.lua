...
local GetProfessions, GetUnitName, UnitClass, GetProfessionInfo, UnitLevel = GetProfessions, GetUnitName, UnitClass,
	GetProfessionInfo, UnitLevel
local ClearCursor, CreateFrame, print, GetCursorInfo = ClearCursor, CreateFrame, print, GetCursorInfo
local ButtonFrameTemplate_HidePortrait, SendMailFrame, UIParent, InboxFrame = ButtonFrameTemplate_HidePortrait,
	SendMailFrame, UIParent, InboxFrame
...
local SetItemButtonDesaturated, SetItemButtonCount, SetItemButtonStock = SetItemButtonDesaturated, SetItemButtonCount,
	SetItemButtonStock
...
function addon:InitData()
	...
	local minLevel = ISCLASSIC and 50 or GetMaxLevelForPlayerExpansion()
	if self:GetNumber("MINLEVEL") > minLevel then
		self:SetVar("MINLEVEL", minLevel - 10)
	end
	self:InitLdb()
...
function addon:OnInitialized()
  ...
  Click on 'Cancel' and restore MailCommander 1.0 if you want to keep the old version
		...
		0, function(this) C_Timer.After(0.5, addon.InitContinue) end,
		function() C_Timer.After(0.5, addon.InitDisabled) end, self)
end
...
function addon:OnInitializedContinue()
	...
	self:AddBoolean("BAGS", true, L["Switch bags with MailCommander"],
		L["Automatically opens and closes bags with MailCommander frame"])
	self:AddLabel(L["Character selection"])
	self:AddSlider("MINLEVEL", 30, 1, maxLevel, L["Characters minimum level"],
		L["Only consider characters above this level"])
		...
		for item, toons in pairs(fromdb.disabled or empty) do
			for sender, receivers in pairs(toons or empty) do
				for receiver, forbidden in pairs(receivers or empty) do
					if forbidden then
						if type(todb.toons[receiver].requests[item]) ~= "table" then
							todb.toons[receiver].requests[item] = {}
				...
				for itemid, info in pairs(db.toons[currentReceiver].requests) do
					...
					if not isdisabled then
						if Count:IsSendable(itemid, bagItemId, toon, bagId, slotId) then
							local n = GetContainerItemInfo(bagId, slotId)
							tobesent[bagItemId] = Count:Sendable(itemid, toon)
							tinsert(sortable, format("%05d:%s:%s:%s", 10000 + bags[bagItemId] - n, bagItemId, bagId,
								slotId))
						end
					end
				end
...
function addon:buildPanel()
	...
	t:AddChild('p1',
		factory:DropDown(t, UNKNOWN, professions, PROFESSIONS_FIRST_PROFESSION,
			CHOOSE .. ' ' .. PROFESSIONS_FIRST_PROFESSION))
	t:AddChild('p2',
		factory:DropDown(t, UNKNOWN, professions, PROFESSIONS_SECOND_PROFESSION,
